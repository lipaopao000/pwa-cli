...
class CitationVerificationResult(BaseModel):
    ...
    local_support: str = Field(
        description="One of: 'Full', 'Partial', 'Not Mentioned', 'Contradictory', 'N/A'"
    )
    factuality: str = Field(
        description="One of: 'Well-established', 'Supported by Evidence', 'Debatable / Mixed', 'Unsupported / No Evidence', 'Contradicted', 'Requires Specification', 'Pending'"
    )
    ...
    status: str = Field(
        description="Internal status code for flow control (e.g., 'Definitive', 'Unsatisfied')."
    )
...
class CitationVerificationAgent:
    def __init__(self, rag_client, pubmed_client, llm_settings, tavily_client=None):
        ...
        self.model = ChatOpenAI(**self.llm_settings)
    ...
    def web_search(self, state: AgentState):
        ...
        try:
            res = self.model.invoke(
                [SystemMessage(content=gen_query_sys), HumanMessage(content=gen_query_user)]
            )
            ...
            try:
                ...
                if abstract and "No Abstract Found" not in abstract:
                    ...
                    new_web_evidences.append(
                        {"citation_key": "PubMed_Result", "content": abstract, "source": "web"}
                    )
                else:
    ...
    def fetch_fulltext(self, state: AgentState):
                    ...
                    "evidences": [
                        {
                            "citation_key": state["citation_id"],
                            "source": "fulltext",
                            "content": "Fulltext content pre-loaded.",
                        }
                    ],
                }
            )
            return {"search_mode": "fulltext", "audit_history": audit_history}
        ...
        try:
            self.logger.info(
                f"Fetching chunks from RAG to reconstruct fulltext for {state['citation_id']}..."
            )
            results = self.rag_client.retrieve(
                dataset_id=state["dataset_id"], query="", document_ids=[state["doc_id"]], top_k=100
            )
            if results:
    ...
    def verify(self, state: AgentState):
        ...
        if mode == "web":
            ...
            if not web_only:
                ...
                return {
                    "verification_result": {
                        "status": "Failed",
                        "reasoning": "No web evidence available for verification.",
                    }
                }
            ...
            current_evidences = [
                ...
                == (
                    ...
                    if mode == "abstract_only"
                        ...
                        "chunk"
                        if mode == "scoped"
                        else "fulltext" if mode == "fulltext" else "global"
                    )
                )
            ]
        ...
        if not current_evidences:
            ...
            if mode == "global":
                result = {
                    "status": "Unclear",
                    "confidence": 0.0,
                    "reasoning": "Global search returned no relevant snippets from any document.",
                }
            else:
                status_if_empty = (
                    "Need More"
                    if mode == "abstract_only"
                    else "Chunk Unsatisfied" if mode == "scoped" else "Not Supported"
                )
                result = {
                    "status": status_if_empty,
                    "confidence": 0.0,
                    "reasoning": f"No content found in {mode} level.",
                }
        ...
        for i, ev in enumerate(current_evidences):
            evidence_lines.append(
                f"--- Evidence {i+1} [Source: {ev['source']}, CitationKey: {ev['citation_key']}] ---\n{ev['content']}"
            )
            ...
            if json_match:
                try:
                    ...
                    try:
                        ...
                        result = {
                            "status": "Error",
                            "reasoning": f"JSON Decode Error: {str(e)} | Content: {content[:200]}",
                            "confidence": 0.0,
                        }
            else:
                result = {
                    "status": "Error",
                    "reasoning": f"Could not parse JSON from LLM: {content[:100]}",
                    "confidence": 0.0,
                }
    ...
    def _decide_next(self, state: AgentState):
        ...
        if mode == "fulltext" and (
            status == "Not Supported" or status == "Failed" or status == "Error"
        ):
            return "next_mode"
    ...
    def finalize_result(self, state: AgentState):
        ...
        if factuality == "True" and support == "Supported" and confidence < 0.6:
            ...
            res["suggestion"] = (
                "置信度较低，建议人工核对：Statement is True but support is questionable."
            )
            support = "Unclear"

        # 1. Update rag_retrieved status
        has_rag_evidence = any(
            e["source"] in ["chunk", "global", "fulltext"] for e in all_evidences
        )
        ...
        if len(audit_history) > 1:
            best_entry = next(
                (e for e in reversed(audit_history) if e["status"] == "Supported"), None
            )
            if best_entry and mode == "global":
                res["reasoning"] = (
                    f"经过多级审计，最终在全库检索中找到支持证据。之前层级（如摘要或目标片段）未能充分支持，原因：{audit_history[0].get('reasoning', '不详')}。最终结论：{res['reasoning']}"
                )
            elif not best_entry:
                res["reasoning"] = (
                    f"经过四级深度审计（摘要->片段->全文->全库），均未能找到充分支持证据。最后一级结论：{res['reasoning']}"
                )

        # 6. Format top evidences for display (Legacy support)
        # We only show evidences from the level that actually provided the result
        current_level_evidences = [
            e
            for e in all_evidences
            if e["source"]
            == (
                ...
                if mode == "abstract_only"
                    ...
                    if mode == "scoped"
                        ...
                        "fulltext"
                        if mode == "fulltext"
                        else "global" if mode == "global" else "web"
                    )
                )
            )
        ]
        ...
        for i, ev in enumerate(display_evidences):
            ...
            evidence_lines.append(
                f"[{ev['source'].upper()}] [@{ev['citation_key']}]\n{ev['content']}"
            )
        res["source_content"] = "\n\n".join(evidence_lines)
