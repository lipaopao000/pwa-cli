...
class Configuration(BaseModel):
    ...
    # Models
    active_provider: str = Field(default="openai", description="The LLM provider to use.")

    model: str = Field(default="gpt-4o-mini", description="The name of the language model to use.")

    temperature: float = Field(default=0.1, description="The temperature for the LLM.")

    # Search Config
    max_web_results: int = Field(
        default=5, description="Maximum number of web search results to fetch."
    )

    # Verification Config
    high_if_threshold: float = Field(
        default=20.0, description="Impact Factor threshold for definitive support."
    )
