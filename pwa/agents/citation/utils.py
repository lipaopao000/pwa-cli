...
def parse_markdown_to_statements(content: str) -> List[Dict[str, Any]]:
    ...
    for i, line in enumerate(lines):
        ...
        if not stripped_line or stripped_line.startswith('#') or (stripped_line.startswith('|') and '---' in stripped_line):
            continue
            ...
            for sent in sentences:
                ...
                if sent_citations:
                    ...
                    if is_scientific_statement(sent_stripped):
                        results.append({"citation_id": None, "context": sent_stripped, "line": line_num})
...
def is_scientific_statement(sent: str) -> bool:
    ...
    is_long_enough = len(sent.split()) > 10 or (any('\u4e00' <= char <= '\u9fff' for char in sent) and len(sent) > 15)
