...
from .schemas import GlobalFactualityEvaluation, LocalSupportEvaluation, SearchQuery
...
class ScientificStatementVerifier:
    ...
    def _build_graph(self):
        ...
        builder.add_conditional_edges(
            "initialize",
            self._route_initial,
            {"citation": "citation_verifier_ladder", "factuality": "factuality_verifier_global"},
        )
    ...
    def citation_verifier_ladder(self, state: AgentState, config: Configuration):
        ...
        if not new_evidences and level != "fulltext":
            ...
            return {"ladder_level": next_level, "audit_history": [audit_entry]}

        # Evaluate support level
        all_current_evidences = state["evidences"] + new_evidences
        eval_result = self._evaluate_local_support(
            state["context"], citation_id, all_current_evidences, config
        )
    ...
    def factuality_verifier_global(self, state: AgentState, config: Configuration):
        ...
        def run_library_rag():
            ...
            if self.rag_client and state.get("dataset_id"):
                try:
                    results = self.rag_client.retrieve(
                        dataset_id=state["dataset_id"], query=query, document_ids=None, top_k=20
                    )
                    for r in results:
                        res.append(
                            {
    ...
    def _evaluate_local_support(
        self, claim: str, citation_id: str, evidences: List[Evidence], config: Configuration
    ) -> LocalSupportEvaluation:
        ...
        return llm.invoke(
            [SystemMessage(content=LOCAL_SUPPORT_SYSTEM_PROMPT), HumanMessage(content=user_msg)]
        )

    def _evaluate_global_factuality(
        self, claim: str, evidences: List[Evidence], config: Configuration
    ) -> GlobalFactualityEvaluation:
        llm = self.default_model.with_structured_output(GlobalFactualityEvaluation)
        evidence_text = self._format_evidences(evidences, claim=claim)

        user_msg = f'Claim: "{claim}"\n\nAll Collected Evidence:\n{evidence_text}'
        return llm.invoke(
            [SystemMessage(content=GLOBAL_FACTUALITY_SYSTEM_PROMPT), HumanMessage(content=user_msg)]
        )
    ...
    def _generate_search_query(self, claim: str) -> str:
        ...
        try:
            res = llm.invoke(
                [SystemMessage(content=SEARCH_QUERY_SYSTEM_PROMPT), HumanMessage(content=claim)]
            )
            return res.query
        except:
            return claim[:100]
    ...
    def _perform_web_search(self, claim: str, config: Configuration) -> List[Evidence]:
        ...
        def search_pubmed():
            ...
            if self.pubmed_client:
                try:
                    pubmed_results = self.pubmed_client.search_details(
                        search_query, max_results=config.max_web_results
                    )
                    ...
                    self.logger.error(f"PubMed failed: {e}")
            return res
            ...
            if self.tavily_client:
                try:
                    search_res = self.tavily_client.search(
                        query=f"scientific evidence for: {search_query}", search_depth="advanced"
                    )
    ...
    def finalize_result(self, state: AgentState):
        ...
        status = (
            ...
            if ls == "Full"
                ...
                "Contradicted"
                if (fac == "Contradicted" or ls == "Contradictory")
                else "Misplaced" if ls in ["Partial", "Not Mentioned"] else "Unclear"
            )
        )
