...
class FullTextDownloadCommand(BaseCommand):
    ...
    def execute(self, ref_source: str, ref_path: Optional[str] = None,
                output_dir: str = "./FullTextMD", max_workers: int = 5,
                **kwargs) -> Dict:
        """
        ...
        try:
            ...
            from ..clients.mineru import MineruClient, FullTextProcessor
...
class FullTextRetryCommand(BaseCommand):
    ...
    def execute(self, max_workers: int = 5, **kwargs) -> Dict:
        ...
        try:
            ...
            # Initialize processor
            processor = FullTextProcessor(
                output_dir="./FullTextMD",
                config_manager=self.config_manager
            )
            
            # Count failed tasks
            failed_count = sum(
                1 for task in processor.task_statuses.values()
