...
class ReferencesMatchCommand(BaseCommand):
    ...
    def get_interactive_params(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"md_file": md_file, "ref_file": ref_file, "top_n": top_n}
    ...
    def execute(
        self, md_file: Path, ref_file: Optional[Path] = None, top_n: int = 3, **kwargs
    ) -> Dict[str, Any]:
        ...
        zotero_cfg_path = self.config_manager.ensure_config(
            "zotero", self.config_manager.config_dir.parent / "configs" / "zotero_config.yaml"
        )

        # Fetch references
        print_info("正在获取参考文献库...")
        references_content, references_type = fetch_preferred_references(
            str(ref_file) if ref_file else None, zotero_config_path=str(zotero_cfg_path)
        )
    ...
    def extract_references_from_md(
        self, md_content: str
        ...
        first_ref_match = re.search(
            r"^\s*(?:(\[\d+\])|(\d+\.)|(\d+\))|(\(\d+\)))\s+", references_text, re.MULTILINE
        )
    ...
    def analyze_references(
        self, md_refs: Dict, bib_data: List[Dict], md_ref_details: Dict, top_n: int = 3
    ) -> Tuple[Dict, List]:
        ...
        bib_doi_map = {
            normalize_doi(item.get("doi", "")): item["ID"] for item in bib_data if item.get("doi")
        }
        ...
        for ref_num, ref_info in md_refs.items():
            ...
            if md_doi_norm and md_doi_norm in bib_doi_map:
                ...
                perfect_matches[ref_num] = {"id": matched_id, "method": "doi", "score": 1.0}
                continue
            ...
            for bib_id, bib_title in bib_titles_map.items():
                ...
                len_ratio = (
                    min(len(md_title) / len(bib_title), len(bib_title) / len(md_title))
                    if len(md_title) > 0 and len(bib_title) > 0
                    else 0
                )
            ...
            if best_match["score"] >= 0.90:
                ...
                nonperfect_matches.append(
                    {"ref_num": ref_num, "md_title": md_title, "candidates": candidates}
                )
    ...
    def _display_results(self, perfect: Dict, nonperfect: List):
        ...
        if perfect:
            ...
            for ref_num, match in perfect.items():
                print(
                    f"  [{ref_num}] → {match['id']} (方法: {match['method']}, 分数: {match['score']:.2f})"
                )
