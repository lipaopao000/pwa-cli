...
class SessionHistoryCommand(BaseCommand):
    ...
    def execute(self, limit: int = 10, **kwargs) -> Dict:
        ...
        for i, entry in enumerate(reversed(history), 1):
            ...
            # Show key parameters
            if params:
                key_params = {k: v for k, v in params.items() if k in ['md_file', 'bib_file', 'output_file']}
                if key_params:
...
class SessionListCommand(BaseCommand):
    ...
    def execute(self, limit: int = 20, **kwargs) -> Dict:
        ...
        for i, session in enumerate(sessions, 1):
            ...
            print(f"     最后活动: {time_str} | 操作数: {session.get_command_count()} | 持续: {self._format_duration(session.get_duration())}")
...
class SessionResumeCommand(BaseCommand):
    ...
    def get_interactive_params(self, context) -> Dict:
        ...
        choice = self.prompt_choice(f"选择要恢复的 Session (1-{len(sessions)})", 
...
class SessionExportCommand(BaseCommand):
    ...
    def get_interactive_params(self, context) -> Dict:
        ...
        session_id = self.prompt_text(
            f"Session ID (默认: 当前 Session)",
            default=default_id
        )
