...
class VerifyStatementsCommand(BaseCommand):
    ...
    def get_interactive_params(self, context) -> Dict:
        ...
        # Get concurrency
        max_workers = self.prompt_text("并发验证数 (默认: 3)", default="3")
        ...
        try:
            ...
            if use_ragflow:
                ...
                rag_client = RagFlowClient(
                    base_url=rag_config["ragflow_base_url"], api_key=rag_config["ragflow_api_key"]
                )
            ...
            if rag_client and dataset_id:
                print_info("\n正在同步文档到 RAGFlow...")
                doc_map = self._sync_documents_to_rag(rag_client, dataset_id, statements)
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in tqdm(as_completed(futures), total=len(statements), desc="验证进度"):
                    try:
                        ...
                        results.append({"error": str(e), "claim_info": {"context": "unknown"}})
    ...
    def _sync_documents_to_rag(self, rag_client, dataset_id: str, statements: List[Dict]) -> Dict:
        ...
        try:
            ...
            unique_keys = set(s.get("citation_id") for s in statements if s.get("citation_id"))
    ...
    def _process_single_statement(
        ...
        try:
            ...
            return {"error": str(e), "claim_info": {"line": line, "context": context}}
    ...
    def _annotate_markdown(self, md_path: str, results: List[Dict], output_dir: Path) -> Path:
        ...
        sorted_results = sorted(
            results, key=lambda x: x.get("claim_info", {}).get("line", 0), reverse=True
        )
...
class VerifyViewResultsCommand(BaseCommand):
    ...
    def get_interactive_params(self, context) -> Dict:
        ...
        results_file = self.prompt_text(f"结果文件路径 (支持通配符)", default=default_file)
...
class VerifyExportReportCommand(BaseCommand):
    ...
    def get_interactive_params(self, context) -> Dict:
        ...
        results_file = self.prompt_text(f"结果文件路径 (支持通配符)", default=default_input)

        default_output = "./verification_report.md"
        output_file = self.prompt_text(
            f"报告文件路径 (默认: {default_output})", default=default_output
        )

        return {"results_file": results_file, "output_file": output_file}
    ...
    def execute(
        self, results_file: str, output_file: str = "./verification_report.md", **kwargs
    ) -> Dict:
        ...
        try:
            ...
            return {"status": "success", "output_file": output_file, "count": len(results)}
